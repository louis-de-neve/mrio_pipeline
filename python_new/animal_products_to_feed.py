...
def animal_products_to_feed(prefer_import='import', conversion_opt='dry_matter', year=2013):
    ...
    output_filename = f"../results/TradeMatrixFeed_{prefer_import}_{conversion_opt}_{year}.csv"
    ...
    animal_product_data_full = animal_trade_data.merge(
        share_per_country,
        left_on=['Year', 'Item_Code', 'Producer_Country_Code'],
        right_on=['Year', 'Animal_Product_Code', 'AP_Producer_Country_Code'],
        how='inner',
        validate='many_to_many').drop(columns=['AP_Producer_Country_Code', 'Animal_Product_Code'])
